...
void shuffle() {
	for (int i = 0; i < 52; i++) {
		...
		int t = temp % 13;
		switch (t) {
		case 0: {Card.head = 12; Card.point = 10; break; }
		...
		case 10: {Card.head = 9; Card.point = 10; break; }
		case 11: {Card.head = 10; Card.point = 10; break; }
		case 12: {Card.head = 11; Card.point = 10; break; }
		}
		t = temp / 13;
		switch (t) {
...
int main() {
	...
	for(int i = 0; i < sum; i++) {
		...
		if (i < sum - 2) std::cout << " , ";
		else if (i == sum - 2) std::cout << " and ";
	}
	int S = Deck[0].point + Deck[1].point;
	std::cout << std::endl << "You have " << S << " point " << std::endl;
	while (1) {
		char d;
		...
		if (d == 'h') {
			...
			std::cout << std::endl;
			S += Deck[sum - 1].point;
			if (S < 21) std::cout << "You have " << S << " point " << std::endl;
			else if(S == 21){
				std::cout << "You have 21 points, You Win " << std::endl;
				break;
			}
			else {
				std::cout << "You have over 21 points, Game over " << std::endl;
				break;
			}
		}
		else if (d == 's') {
